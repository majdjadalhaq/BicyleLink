...
const StickyContactBar = ({ listing, displayPrice, isOwner, user, id }) => {
  ...
  // Sold listing visible to non-owner: show a "see similar" CTA
  if (listing.status === "sold" && !isOwner) {
    return (
      <div className="fixed bottom-14 left-0 right-0 z-[40] md:hidden bg-white/95 dark:bg-[#1a1a1a]/95 backdrop-blur-md border-t border-gray-200 dark:border-[#2a2a2a] px-4 py-3 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] dark:shadow-[0_-4px_30px_rgba(0,0,0,0.2)]">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          <div className="flex flex-col">
            ...
            <span className="text-lg font-black text-gray-400 dark:text-gray-500 leading-none line-through">
              €{displayPrice}
            </span>
          </div>
          <Link
            to={`/?category=${listing.category || ""}`}
            className="flex-1 max-w-[220px] py-3 bg-gray-800 dark:bg-white/10 text-white text-center rounded-xl font-black text-xs uppercase tracking-widest active:scale-[0.98] transition-all"
          >
            See Similar Bikes
          </Link>
        </div>
